...
| `worked-moment-with-zone-pill-1280x800.png`     | The same scene right after a re-entry into the vent, chip `309 ▼ 10/s` (the toxin drained it over the stepped ticks): all three tags and `● WARM VENT · DECAY ×1.5 · ORANGE RODS` under the orbit.                                                                                                                   |
| `zone-pill-entry-1280x800.png`                  | The zone pill on entry, chip `309 ▼ 0.7/s`, with two tags (the toxic contact had just ended that tick).                                                                                                                                                                                                              |
...
| `cap-rate-column-1024x640.png`                  | The cap on the smallest viewport, chip `4994 ▼ 163/s`: a full three-tag column (`−150/s TOXIN`, `−8.5/s DECAY`, `−4.2/s VENT`). The top pill's upper edge is at about y 105 (read from a 2× crop), under the notice stack's 96 — the §3.1.5 inequality's 105.0.                                                      |
