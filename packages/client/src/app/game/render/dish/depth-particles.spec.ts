...
describe('depth particles', () => {
  ...
  it('moves with the camera at the layer parallax: the far layer lags, the bokeh leads', () => {
    const particle = {
    ...
    expect(positionAt('far', 100) - positionAt('far', 0)).toBeCloseTo(100 * (1 + (1 - DEPTH_PARALLAX.far)), 6);
    expect(positionAt('bokeh', 100) - positionAt('bokeh', 0)).toBeCloseTo(100 * (1 + (1 - DEPTH_PARALLAX.bokeh)), 6);
  });
});
