...
// scaled to the dish, the wall's crisp lines as world-scale Graphics (the one hard edge), the
// three depth particle layers, and the screen-space vignette the orchestrator keeps above
// everything. The vent shimmer joins with a later slice (see the PR's deferred list).
...
export class DishLayer {
  ...
  constructor(dishField: DishField, glowTexture: Texture, cosmetic: RandomSource) {
    this.field = new Sprite(textureFromBake(dishField.canvas));
    this.field.anchor.set(HALF);
    const extent = dishField.halfExtentWu * DIAMETER_PER_RADIUS;
    this.field.width = extent;
    this.field.height = extent;
