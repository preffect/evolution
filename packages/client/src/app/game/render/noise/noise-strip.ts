...
export interface NoiseStrip {
  ...
  /** Rest lobes per row (the §9 5–7 pin). */
...
function jitterAt(knots: readonly number[], theta: number): number {
  const position = wrapUnit(theta / RADIANS_PER_FULL_TURN) * knots.length;
...
export function buildNoiseStrip(cosmetic: RandomSource): NoiseStrip {
  ...
  const stripRandom = cosmetic.fork(COSMETIC_SUB_STREAM.strip);
  for (let row = 0; row < NOISE_STRIP_ROWS; row += 1) {
    const rowRandom = stripRandom.fork(`${COSMETIC_SUB_STREAM.strip}:${row}`);
...
export function sampleNoiseStrip(strip: NoiseStrip, row: number, unit: number): NoiseStripSample {
  ...
  const texel = wrapUnit(unit) * strip.width - HALF;
  ...
  const read = (column: number, highChannel: number, lowChannel: number, scale: number): number => {
    const offset = (wrappedRow * strip.width + column) * RGBA_CHANNELS;
    return decodeSigned(strip.bytes[offset + highChannel] ?? 0, strip.bytes[offset + lowChannel] ?? 0, scale);
  };
  const channel = (highChannel: number, lowChannel: number, scale: number): ChannelRead => {
