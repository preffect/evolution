...
// 0.40 r soft glow, the 0.30 r disc's rim, chromatin spots scattered from the cosmetic
// `organelles` sub-stream, a white nucleolus with its own halo and the nucleus's own highlight
// inside the disc — and no disc fill: the disc is the cell shader's nucleus ramp under the sprite
// (#231, VISUAL-STYLE §3), so the palette's own ramp shows through. The nucleoid is a loose glowing
// loop of thread wobbling on two incommensurate terms at seeded phases. Both are baked white below
// full alpha so the layer tints them with the palette's nucleus / rim colour and the highlight still
// reads as a lighter spot.
...
/** Sheet 01 layer 6 minus its disc fill; tinted by the palette's nucleus colour at draw time. */
