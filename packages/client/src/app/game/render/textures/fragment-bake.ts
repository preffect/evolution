...
/** A line at the atlas scale keeps its px width; a smaller variant keeps this floor. */
const LINE_WIDTH_FLOOR_PX = 1;
...
function lineWidthPx(basePx: number, frame: HelixFrame): number {
  return basePx * frame.pxScale + LINE_WIDTH_FLOOR_PX;
}

function paintRungs(context: BakeContext2D, frame: HelixFrame, colour: string): void {
  context.lineCap = 'round';
  context.strokeStyle = colour;
  context.globalAlpha = FRAGMENT_BAKE.rungAlpha;
  context.lineWidth = lineWidthPx(DNA_FRAGMENT_RUNG_PX, frame);
  for (let rung = 0; rung < DNA_FRAGMENT_RUNGS; rung += 1) {
    ...
    const rise = Math.sin(share * Math.PI * FRAGMENT_BAKE.helixTurns) * frame.height * HALF;
    context.beginPath();
...
function paintStrand(context: BakeContext2D, frame: HelixFrame, colour: string, phase: number): void {
  context.strokeStyle = colour;
  context.globalAlpha = FRAGMENT_BAKE.strandAlpha;
  context.lineWidth = lineWidthPx(FRAGMENT_BAKE.strandPx, frame);
  context.beginPath();
  for (let step = 0; step <= frame.length; step += 1) {
    const share = step / frame.length;
    const y = frame.centre + Math.sin(share * Math.PI * FRAGMENT_BAKE.helixTurns + phase) * frame.height * HALF;
...
export function bakeFragmentSprite(factory: BakeCanvasFactory, tag: DnaTag, pxPerWu: number): BakeCanvas {
  ...
  const size = Math.ceil(Math.max(length, halo * DIAMETER_PER_RADIUS) + halo);
  ...
  paintRungs(context, frame, tagColour);
