...
describe('GhostRegistry', () => {
  it('keeps a ghost for the absorbed clip and drops it at 600 ms', () => {
    ...
    const prey = createTestCellView({ id: entityId('prey'), x: 0, y: 10 });
    registry.add(createTestGhostSource({ view: prey, speckleSeed: 0.5 }), predator, 1000);
    ...
    expect(early[0]!.speckleSeed).toBe(0.5);
  ...
  it('drives the predator seal from the ghost: 0.60 at the seal, relaxing to 0 at done, the largest winning', () => {
    const registry = new GhostRegistry();
    registry.add(createTestGhostSource({ view: createTestCellView({ id: entityId('a'), x: 10 }) }), predator, 0);
    registry.add(createTestGhostSource({ view: createTestCellView({ id: entityId('b'), y: -10 }) }), predator, 300);
