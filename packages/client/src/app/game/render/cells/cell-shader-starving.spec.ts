...
describe('cell shader source: a starving cell', () => {
  it('withers every palette shade of a starving cell toward its sallowed, dimmed grey by the wither (#635)', () => {
    const shade = CELL_FRAGMENT_SOURCE.slice(CELL_FRAGMENT_SOURCE.indexOf('vec3 shade(Instance inst, int column) {'));
    expect(shade.slice(0, shade.indexOf('\n}'))).toContain('return withered(texelFetch(uPalette');
