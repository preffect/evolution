...
// flows into an engulf arm in proportion to ρ and stretches with the body.
...
export interface MappedPoint {
  ...
  readonly x: number;
  readonly y: number;
  /** `r(θ)` at the slot's angle, for a sprite that scales with the local membrane. */
...
/** The slot at `(x, y)` (cell frame, fractions of `r`) mapped through the profile `terms`. */
export function mapSlot(x: number, y: number, terms: RadialProfileTerms): MappedPoint {
  const distance = Math.hypot(x, y);
  if (distance === 0) return { x: 0, y: 0, localRadius: evaluateProfile(terms, 0).r };
  const theta = Math.atan2(y, x);
  const localRadius = evaluateProfile(terms, theta).r;
  return { x: (x / distance) * distance * localRadius, y: (y / distance) * distance * localRadius, localRadius };
}
