...
import { DNA_RING_KEEP_OUT_FRACTION, NUCLEUS_RADIUS, ORGANELLE_MEMBRANE_MARGIN, ORGANELLE_MIN_GAP } from '../constants';
...
const isNucleus = (slot: OrganelleSlot) =>
  slot.kind === ORGANELLE_KIND.nucleus || slot.kind === ORGANELLE_KIND.nucleoid;
...
describe('layoutOrganelles', () => {
  it('places the nucleus 0.12 r toward the light and every organelle in the annulus, clear of the disc', () => {
    ...
    expect(distanceOf(nucleus)).toBeCloseTo(0.12, 9);
    ...
    for (const slot of slots.filter((candidate) => !isNucleus(candidate))) {
      expect(distanceOf(slot)).toBeGreaterThanOrEqual(DNA_RING_KEEP_OUT_FRACTION - 1e-9);
      expect(distanceOf(slot)).toBeLessThanOrEqual(1 - ORGANELLE_MEMBRANE_MARGIN + 1e-9);
  ...
  it('holds the gap between organelle sprites', () => {
    ...
    for (const first of slots) {
      for (const second of slots) {
        ...
        expect(gap).toBeGreaterThanOrEqual(ORGANELLE_MIN_GAP - 1e-9);
      }
    }
  });
  ...
  it('gives a protocell granules and no nucleus, and a prokaryote a nucleoid and lipids', () => {
    ...
    expect(kinds).toEqual([...Array(3).fill(ORGANELLE_KIND.protocellGranule)]);
    const prokaryote = summariseCellTraits(
