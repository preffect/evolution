// The preview evidence route (docs/architecture/encyclopedia.md §12.7, §12.9): the query it parses and the anchor it
// joins to a render spec; its production gate is `preview-route-gate.spec.ts`. The walk arithmetic and the budget
// verdict are `render/preview/preview-timings.spec.ts`: `encyclopedia/` reads no tunable from a module import (§12.6).
//
