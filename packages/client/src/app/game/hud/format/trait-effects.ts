...
// instead of rendering `undefined`. A value at its identity says nothing and is skipped; every other one is a line,
// in the tier row's own order, so a card never hides a trait's cost. The spec pins that no tier row has more than
...
const UNCHANGED = 1;
/** The one range that reads in the singular. */
const ONE_RADIUS = 1;
...
/** `1 radius`, `1.5 radii`. */
function radiiText(value: number): string {
  const radii = trimmed(value);
  return `${radii} ${radii === ONE_RADIUS ? 'radius' : 'radii'}`;
}

/** A multiplier read as a change from 1: 1.15 → `+15 %`, 0.85 → `−15 %`. */
const fromOne = (value: number): string => signedPercent(value - UNCHANGED);
/** A duration multiplier read as the rate it gives: 0.61 of the time is `1 / 0.61 − 1` = `+64 %` faster. */
const asRate = (value: number): string => signedPercent(UNCHANGED / value - UNCHANGED);
...
/** The modifiers of `traitId`'s tier row (1..3) that differ from identity, in the row's order. */
export function nonIdentityModifiers(traitId: TraitId, tier: number): [ModifierKey, number][] {
  const row: Partial<CellModifiers> = TRAIT_TIERS[traitId]?.[Math.max(tier, FIRST_TIER) - FIRST_TIER] ?? {};
...
export function describeTierModifiers(traitId: TraitId, tier: number): string[] {
  return nonIdentityModifiers(traitId, tier).map(([key, value]) => MODIFIER_LABELS[key](value));
}
