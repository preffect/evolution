// What the input layer remembers between client ticks (docs/ui/input-and-onboarding.md §4): the latched pointer, the
...
export interface InputState {
  /** The pointer's last position over the canvas; latched when it leaves (docs/ui/input-and-onboarding.md §4). */
  ...
  /** Tab is held, so the full leaderboard is open (docs/ui/input-and-onboarding.md §4). */
...
/** Clears the sprint one-shot once it has been sent: one sprint per press (docs/ui/input-and-onboarding.md §4). */
 ...
 * the results phase). A press made then is not carried into the next round (docs/ui/input-and-onboarding.md §4).
 */
