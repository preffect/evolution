...
/**
 * Opens the encyclopedia (docs/ui/encyclopedia.md §11.7, docs/ui/input-and-onboarding.md §4), edge-triggered: from
 * play, returning to the game, and from the menu, returning to the menu. It lives here rather than with the
 * encyclopedia's own key codes because only the room's input layer reads it — the lobby has no document listener.
 */
export const ENCYCLOPEDIA_KEY_CODE = 'KeyH';
...
/** The encyclopedia panel (docs/ui/encyclopedia.md §11.1): modal, and the one overlay `H` does not act from. */
export const ENCYCLOPEDIA_OVERLAY_TEST_ID = ENCYCLOPEDIA_TEST_ID.encyclopedia;

/**
 * The modal overlays (docs/ui/input-and-onboarding.md §4's modal gate): while one is open only `1` `2` `3`, Escape
 * and — with the menu open — `H` reach past it, which leaves the arrow keys to the kit's rails and lists.
 */
export const MODAL_OVERLAY_TEST_IDS: readonly string[] = [MENU_OVERLAY_TEST_ID, ENCYCLOPEDIA_OVERLAY_TEST_ID];
