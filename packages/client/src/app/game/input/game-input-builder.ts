// One client tick's `GameInput` (docs/architecture/wire-contract.md §4, docs/ui/input-and-onboarding.md §4): the latched pointer or
...
export interface InputWorldContext {
  ...
  /**
   * The newest `sequence` the server has applied for this player (docs/architecture/wire-contract.md §4). A
...
export interface GameInputBuildOptions {
  ...
  /** The client tick this input is for: monotonic, echoed back for prediction (docs/architecture/client.md §5). */
 ...
 * (docs/game-design/controls-and-scope.md §6), so a held key is full throttle exactly as a distant pointer is.
 */
