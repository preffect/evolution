...
// when the manifest or a file is missing; never throws into the game loop. Only `SoundEventBus`
// calls it; the HUD reaches the mute through it as well.
...
export class AudioService implements SoundSink {
  ...
  setDanger(isDanger: boolean): void {
    ...
    if (isDanger) {
      ...
      this.guard(() => this.ambient.duck('danger'));
    } else {
      this.stopLoop(SOUND_EVENT.dangerWarning);
      this.guard(() => this.ambient.release('danger'));
    }
  }
  ...
  private playOneShot(id: SoundEventId, variantKey: string | undefined): void {
    ...
    if (rule.priority === SOUND_PRIORITY.essential) {
      this.ambient.duck('essential_cue');
      voice.onEnded(() => this.ambient.release('essential_cue'));
    }
  }
  ...
  private startVoice(id: SoundEventId, start: VoiceStart): AudioVoice | null {
    const fileName = this.resolveFileName(id, start.variantKey);
  ...
  private resolveFileName(id: SoundEventId, variantKey: string | undefined): string | null {
