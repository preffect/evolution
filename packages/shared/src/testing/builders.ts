...
// (`createTestCell`, `createTestWorld`) are built in the server package, beside the records.

import { CELL_STAGE, GAME_MODE, PLAYER_LIFE_STATE, ROUND_END_CONDITION, ROUND_PHASE } from '../types/game.js';
...
import { CELL_STARTING_MASS, CELL_RADIUS_SCALE } from '../constants/growth.js';
...
/** A free protocell at the origin at starting mass; pass the fields the test reads. */
export function createTestCellView(overrides: Partial<CellView> = {}): CellView {
  ...
  return {
    ...
    velocityY: 0,
    mass,
    radius: CELL_RADIUS_SCALE * Math.sqrt(mass),
    level: 1,
