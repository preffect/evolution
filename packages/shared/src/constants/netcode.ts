...
/**
 * How long a client may go between acknowledgements. A duration, not a count of snapshots (#277):
 * `SNAPSHOT_BACKLOG_LIMIT_TICKS` is a second of ticks, so a cadence counted in snapshots would move
 * the floor of that measurement every time `SNAPSHOT_EVERY_TICKS` moved, and one lever would eat
 * the other's headroom. A fixed 5 snapshots is 5 ticks at 60 Hz, 15 at 20 Hz and 20 at 15 Hz, all
 * against the same 60-tick limit. Derived from a tenth of a second it is 6, 6 and 4.
 */
const SNAPSHOT_ACK_INTERVAL_MS = 100;

/**
 * The interval above in snapshots, which is what the client can count. Rounded **down**, so the
 * acknowledgement never falls further apart than the interval at a cadence that does not divide it
 * (at 15 Hz, rounding up would be 133 ms); at least one, for a cadence slower than the interval.
 */
export const SNAPSHOT_ACK_EVERY_SNAPSHOTS = Math.max(
  1,
  Math.floor(SNAPSHOT_ACK_INTERVAL_MS / (SNAPSHOT_EVERY_TICKS * TICK_INTERVAL_MS)),
);
