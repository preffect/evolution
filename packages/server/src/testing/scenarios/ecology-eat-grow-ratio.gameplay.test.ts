...
/** Far east of the broth point: steering away from it turns the second leg west, back over the ground the first covered. */
const WEST_TURN_POINT = { x: BROTH_POINT.x + 100_000, y: BROTH_POINT.y };
...
describe('docs/ecology/mass-and-movement.md §5: eating grows the cell and the growth slows it', () => {
  it('a starting cell eats to 80 mass, then to 320, and its top speed falls with each meal', async () => {
    ...
    await run
      ...
      .expect('the 80-mass cell tops out at its own cap, under the starting cell’s', (view) => speedOf(view, 0))
      .atTick(LEG_TICKS)
      ...
      .toBeCloseTo(SECOND_MEAL_MASS, MASS_TOLERANCE)
      .expect('the 320-mass cell tops out lower again', (view) => speedOf(view, 0))
...
describe('docs/ecology/absorption.md §6.1: the engulf ratio at its edges', () => {
  it.each([
    ['just over', START_EDGE_MASS + EDGE_MARGIN_MASS, beingEngulfed],
    ['just under', START_EDGE_MASS - EDGE_MARGIN_MASS, []],
  ])('%s 1.25 × a 20-mass prey: the engulf starts on tick 1 or never', async (_edge, predatorMass, expectedStates) => {
    await engulfPair(`start edge ${predatorMass}`, predatorMass)
  ...
  it.each([
    ['just over', HEAVY_START_EDGE_MASS + EDGE_MARGIN_MASS, beingEngulfed],
    ['just under', HEAVY_START_EDGE_MASS - EDGE_MARGIN_MASS, []],
  ])('%s 1.25 × a 400-mass prey: the threshold scales with the prey', async (_edge, predatorMass, expectedStates) => {
    await engulfPair(`heavy start edge ${predatorMass}`, predatorMass, HEAVY_PREY_MASS)
      .advance(EDGE_RUN_TICKS)
      .expect('the first eligibility check decides it', statesOfPrey)
      .atTick(1)
      .toEqual(expectedStates)
  ...
  it('just over 1.10 × the prey, an engulf in progress holds, though it could not start there', async () => {
    await releaseEdgePair('release edge held')
      ...
      .atEnd()
      .toBeGreaterThan(0)
      .expect('no release of any kind', releaseReasons)
      .atEnd()
      .toEqual([])
      .runDeterministic();
  });

  it('just under 1.10 × the prey, the engulf releases on the ratio the tick the mass drops', async () => {
    await releaseEdgePair('release edge dropped')
      ...
      .expect('free, and it cannot restart under 1.25', statesOfPrey)
      .atEnd()
      .toEqual([])
