...
describe('ecology/acceptance.md §8: the engulf lifecycle on placed cells (#258; the payout is #259)', () => {
  ...
  it('E9b: a predator steering away drags its cover, and grabbing costs it no speed (#634)', async () => {
    await engulfPair('E9b')
      ...
      .atTick(E9_SEAL_TICK)
      .toBeCloseTo(absorption.ENGULF_SEAL_PROGRESS, PROGRESS_TOLERANCE)
      .expect('centres 29.72 wu apart at the seal, inside the predator reach', (view) =>
        distanceBetweenCells(view, 0, 1),
      )
      .atTick(E9_SEAL_TICK)
      .toBeCloseTo(E9B_SEAL_DISTANCE_WU, DISTANCE_TOLERANCE_WU)
      .expect('predator 19.72 wu west of its start on tick 18', (view) => (cellOf(view, 0)?.x ?? 0) - BROTH_POINT.x)
      .atTick(E9_SEAL_TICK)
      ...
      .expect('speed 104.6 wu/s on tick 18, uncapped while it wraps', (view) => speedOf(view, 0))
      .atTick(E9_SEAL_TICK)
      .toBeCloseTo(E9B_SPEED_TICK_18, SPEED_TOLERANCE_WU_PER_SECOND)
      .expect('speed 107.5 wu/s on tick 19, the first sealed tick: no step at the seal', (view) => speedOf(view, 0))
      .atTick(E9_SEAL_TICK + 1)
      .toBeCloseTo(E9B_SPEED_TICK_19, SPEED_TOLERANCE_WU_PER_SECOND)
      .expect('speed 134.0 wu/s on tick 35, uncapped', (view) => speedOf(view, 0))
      .atTick(E9_PAYOUT_TICK - 1)
