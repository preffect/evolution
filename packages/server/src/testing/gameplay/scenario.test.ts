...
import { BROTH_POINT, PLACED_KIND, type PlacedFixture } from './fixtures.js';
import { createScenarioDsl, DEFAULT_HASH_EVERY_TICKS, player, scenarioPlayerId } from './scenario.js';
...
describe('scenario builder', () => {
  ...
  it('numbers players, ids and names by index and stamps joins and leaves', () => {
    const { players } = toyScenario('roster').seed(42).players(2).playerJoinsAt(10).playerLeavesAt(20, 1).build();
  ...
  it('rejects scripts for unknown players, inputs at tick 0 and bad tick counts', () => {
  ...
  it('registers expectations with their tick, label and matcher', () => {
    const { expectations } = toyScenario('expectations')
      .seed(42)
      .players(1)
      .expect('x', () => 1)
      .atTick(3)
  ...
  it('rejects an expectation at a negative or fractional tick', () => {
  ...
  it('places cells, motes and fragments by the ECOLOGY §8 convention', () => {
    const { fixtures } = placingScenario('E9')
      .seed(42)
      .players(2)
      .placeCell({ playerIndex: 0, mass: 100 })
      .placeCell({ playerIndex: 1, mass: 20, eastOfFirstCellWu: 10 })
      .placeMote({ moteKind: 'algae', eastOfFirstCellWu: 0 })
      .placeFragment({ tag: 'sensory', at: { x: 1, y: 2 } })
      .build();
    expect(fixtures.map((fixture) => [fixture.kind, fixture.at])).toEqual([
      [PLACED_KIND.cell, BROTH_POINT],
      [PLACED_KIND.cell, { x: 1510, y: 0 }],
      [PLACED_KIND.mote, BROTH_POINT],
      [PLACED_KIND.fragment, { x: 1, y: 2 }],
    ]);
  });

  it('refuses to place a cell for a player that does not exist', () => {
    expect(() => placingScenario('E9').seed(42).players(1).placeCell({ playerIndex: 1, mass: 20 })).toThrow(
      ScenarioSetupError,
    );
  });

  it('keeps the script a player was given', () => {
    const script = targetPoint(1, 2);
    const { scripts } = toyScenario('script').seed(42).players(1).atTick(1, player(0).does(script)).build();
    expect(scripts[0]?.script).toBe(script);
  });
});
