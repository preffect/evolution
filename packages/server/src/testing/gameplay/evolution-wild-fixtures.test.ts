// docs/testing/scenario-runner.md §8.1 `.placeWildCell` on a live world: the seat's size factor, a cell at the anchor at
...
describe('applyPlacedWildCell', () => {
  it("seats seat 0's cell at the centre at rest, at its base size with no growth, and clears the seat's residue", () => {
    ...
    const cell = applyPlacedWildCell(
      world,
      placeWildCell({ seat: SEAT, sizeFactor: SIZE_FACTOR }, undefined),
      BROTH_POINT,
    );
    expect(seat.cellId).toBe(cell.id);
    expect(seat.sizeFactor).toBe(SIZE_FACTOR);
    expect(seat.grownMass).toBe(0);
    expect(seat.fullMass).toBe(growth.CELL_STARTING_MASS * SIZE_FACTOR);
  ...
  it('withdraws the cell the seat had without detritus, aborting the engulf it was in', () => {
    ...
    applyPlacedWildCell(world, placeWildCell({ seat: SEAT, sizeFactor: 1 }, undefined), BROTH_POINT);
  ...
  it('creates the seat record on demand in a placed row, which has no seeded seats, in seat order', () => {
    ...
    applyPlacedWildCell(world, placeWildCell({ seat: 3, sizeFactor: 1 }, undefined), BROTH_POINT);
    const cell = applyPlacedWildCell(
      world,
      placeWildCell({ seat: SEAT, sizeFactor: SIZE_FACTOR }, undefined),
      BROTH_POINT,
    ...
    expect(world.wildSeats[0]).toMatchObject({ cellId: cell.id, sizeFactor: SIZE_FACTOR });
  ...
  it('refuses a seat number the dish has not', () => {
    const fixture = placeWildCell({ seat: wildCells.WILD_CELL_COUNT, sizeFactor: 1 }, undefined);
