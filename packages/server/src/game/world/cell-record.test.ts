...
import { CELL_KIND, DEFAULT_CELL_MODIFIERS, NO_STEER_COMMAND, STARTING_STAGE, WORLD_ORGANISM_ID, entityId, playerId } from '@evolution/shared';
...
describe('bornCellRecord', () => {
  it('is free, at rest and underived, carrying the identity it was given', () => {
    const cell = bornCellRecord(
      { id: entityId('c-9'), kind: CELL_KIND.player, playerId: playerId('p1'), organismId: entityId('c-9'), avatarIndex: 3, level: 4 },
      { x: 10, y: -20 },
      50,
    );
    expect(cell).toMatchObject({ id: 'c-9', kind: 'player', playerId: 'p1', organismId: 'c-9', avatarIndex: 3, level: 4 });
    ...
    expect(cell).toMatchObject({ targetX: null, targetY: null, pinnedX: null, carriedOffsetX: null, lastRelease: null });
  ...
  it('makes a wild cell with no player and the world organism', () => {
    const cell = bornCellRecord(
      { id: entityId('c-2'), kind: CELL_KIND.wild, playerId: null, organismId: WORLD_ORGANISM_ID, avatarIndex: 0, level: 1 },
      { x: 0, y: 0 },
      20,
    );
