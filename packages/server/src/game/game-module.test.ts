...
describe('the echo module drives its own bots (docs/architecture/debug-mcp.md §8)', () => {
  ...
  it('spawnBot adds a player whose input the module produces itself, one per tick from tick 1', () => {
    const game = defaultGameModuleFactory(options);
    const bot = game.getDebugHandle!().spawnBot!(
      createTestBotSpawnRequest({ behavior: 'wander', seed: 42 }),
      seatFreely,
    );
  ...
  it('two modules with the same seed drive their bots identically', () => {
    const inputAfterTicks = () => {
      const game = defaultGameModuleFactory(options);
      game.getDebugHandle!().spawnBot!(createTestBotSpawnRequest({ behavior: 'wander', seed: 7 }), seatFreely);
  ...
  it('spawnBot claims the seat before holding the player, so a refused seat leaves the module and roster untouched', () => {
    ...
    expect(() => handle.spawnBot!(createTestBotSpawnRequest({ behavior: 'wander', seed: 42 }), refuse)).toThrow(
      DebugRequestError,
    );
