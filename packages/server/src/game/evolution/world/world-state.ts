...
import type { CellRecord, DnaFragmentRecord, FoodMoteRecord, PlayerRecord, SpawnerState } from './entities.js';
...
export interface WorldState {
  ...
  /** Whole ticks since the round started: the integer the timer boundaries compare (docs/GAME-DESIGN.md §5.4). */
  roundElapsedTicks: number;
  /** Derived from `roundElapsedTicks` every tick and carried for the snapshot. */
  roundTimeLeftMs: number;
  /** Whole ticks spent in the results phase. */
  resultsElapsedTicks: number;
