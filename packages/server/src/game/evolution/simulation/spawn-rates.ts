...
import { FOOD_KIND, type BalanceConfig } from '@evolution/shared';
...
/**
 * The per-event kind weights: `FOOD_KIND_WEIGHTS` are per-mote shares and a bacterium event
 * spawns a whole cluster, so its event weight is the share over the cluster size (docs/ECOLOGY.md
 * §3: 0.75 : 0.05, renormalised by the draw). Index 0 is algae, index 1 a bacterium cluster.
 */
export function spawnEventKindWeights(balance: BalanceConfig): readonly [number, number] {
  const shares = balance.ecology.FOOD_KIND_WEIGHTS;
