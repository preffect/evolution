...
function spawnBacteriumCluster(world: WorldState, random: RandomSource, centre: Vec2, limits: ClusterLimits): number {
  const { ecology } = world.balance;
  const zone = zoneAt(centre, world.gelPatches, world.balance);
  const variantWeights = ecology.BACTERIUM_VARIANT_WEIGHTS_BY_ZONE[zone];
...
export function spawnFoodEvent(world: WorldState, random: RandomSource, room: number, maxAttempts: number): number {
  const kind = SPAWN_EVENT_KINDS[random.weightedIndex(spawnEventKindWeights(world.balance))] ?? FOOD_KIND.algae;
  const point = drawPointInZone(world, drawZone(kind, random, world.balance), random, maxAttempts);
  if (point === null) {
    return 0;
  }
  if (kind === FOOD_KIND.bacterium) {
    return spawnBacteriumCluster(world, random, point, { room, maxAttempts });
  }
...
/** Step 8: both spawners for one tick. */
export function runSpawners(world: WorldState, context: StepContext): void {
  ...
  runSpawner(
    ...
    () => world.food.length,
    (room) => spawnFoodEvent(world, context.streams[RANDOM_STREAM.spawner], room, maxAttempts),
  );
  runSpawner(
...
export function runInitialFill(world: WorldState, context: StepContext): void {
  ...
  fill(
    ...
    () => world.food.length,
    (room) => spawnFoodEvent(world, context.streams[RANDOM_STREAM.spawner], room, INITIAL_FILL_POINT_MAX_ATTEMPTS),
    world.spawners.food,
  );
  fill(
