...
describe('spawner rates', () => {
  ...
  it('multiply the rates, not the caps, during the bloom', () => {
    ...
    world.roundElapsedTicks = bloomStartTick(world, DEFAULT_BALANCE);
...
describe('spawnEventKindWeights', () => {
  it('is the per-mote algae share against the bacterium share over the cluster size', () => {
    expect(spawnEventKindWeights(DEFAULT_BALANCE)).toEqual([0.75, 0.05]);
  });
});
