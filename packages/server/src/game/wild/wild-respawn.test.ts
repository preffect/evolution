...
// zero, at the world's mass × a fresh spread; the seat count never changes.
...
function eatenSeat(): { world: WorldState; eatenCellId: EntityId } {
  ...
  storeStreams(world, context.streams); // as the step would: the respawn draws the next spread, not this one again
...
describe('runWildRespawns (W4)', () => {
  ...
  it('is alive again on tick 637 at the world mass × a fresh spread, with a new cell id', () => {
    const { world, eatenCellId } = eatenSeat();
    const seat = world.wildSeats[0]!;
    const spreadBefore = seat.massSpreadFactor;
    ...
    expect(seat.respawnInTicks).toBe(0);
    // 637 ticks → 10.617 s → world mass 30.617, spread within [21.43, 39.80].
    expect(cell.mass).toBeCloseTo(worldReferenceAt(world, aliveTick).worldMass * seat.massSpreadFactor, 10);
