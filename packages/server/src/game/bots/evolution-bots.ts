// The bots the Evolution module drives itself (`debug_spawn_bot`, docs/architecture/debug-mcp.md §8): an
// in-process roster bound to the room's live balance, stepped before each tick on the live world as the tick before
// left it (no snapshot is built for them, #181), its inputs stamped with the coming tick as their sequence
// (docs/testing/scenario-runner.md §8.1: inputs start at tick 1). The roster owns the pilots; the module owns the
// players and records their inputs like anyone else's, so a replay needs no bot.

import type { GameInput, PlayerId } from '@evolution/shared';
import { createEvolutionWorldBotBinding } from './evolution-binding.js';
import { createInProcessBotRoster, type InProcessBotRoster } from './in-process-bots.js';
import type { WorldState } from '../world/world-state.js';

export type EvolutionBotRoster = InProcessBotRoster<GameInput, WorldState>;
export type InputSubmitter = (playerId: PlayerId, input: GameInput) => void;

export function createEvolutionBotRoster(world: WorldState): EvolutionBotRoster {
  return createInProcessBotRoster(createEvolutionWorldBotBinding(() => world.balance));
}

/** Every bot decides on the world as it stands and submits for the coming tick. */
export function driveBots(roster: EvolutionBotRoster, world: WorldState, submit: InputSubmitter): void {
  roster.driveTick(world, world.tick + 1, submit);
}
