...
function registerRoomPerformanceTool(mcp: McpServer, context: DebugContext): void {
  mcp.tool(
    ...
    'Get per-room tick timings (step + snapshot broadcast; the broadcast also on its own: broadcastAvgMs over every tick, broadcastP95Ms over broadcast ticks), snapshot byte sizes, broadcast fan-out and bytes/sec (tick broadcasts and resync game_states; not the off-tick debug step/republish frames), and snapshotFlow: resyncs sent, players owed one, and backlogTicks per seated connection (null = no ack since its stream (re)started). Omit gameId for all active rooms.',
