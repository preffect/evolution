...
// with the wire types; `types` never imports `simulation` (ARCHITECTURE §10)
...
`CellModifiers` carries every field named above (TRAITS §2), so the arguments are literal `Pick`s of the
...
| Reserved | trait steal: `ENGULF_TRAIT_STEAL_CHANCE` = 0 in build 1, declared so build 2 can turn it on. No draw is made while the chance is 0, the same rule the spit-out follows, so a dish that engulfs and one that does not keep the `engulf` stream in step. **When build 2 turns it on** it draws `streams.engulf` in the payout itself, after that tick's spit-out draw, exactly one draw per completed engulf and none when the chance is 0: that is the draw order the replay of a build-2 round depends on. Because each stream is forked from the round seed by label and never from another stream's sequence ([`DETERMINISM.md §3`](./DETERMINISM.md#3-seeded-random-streams-packagessharedsrcrandom-73)), turning it on can only shift later draws inside `engulf`; `spawner`, `wildCells`, `traitDraft` and the rest are untouched. |
...
| E10  | seed 42, A placed at mass 24, B at 20, centres 10 wu apart                                                                                                                                                                                                                                                                    | idle                                                           | 120 ticks                     | no engulf (24 < 25 and decaying); separation has pushed them apart: `A.radius + B.radius − distance` < 0.01 wu (overlap × 0.8^120) — **unreached through the whole step, see #261**: an idle placed cell steers back to its latched target, which balances the separation at ≈ 2.96 wu of overlap. With A placed at 26 (≥ 25 on tick 1 after decay, and ≥ `ENGULF_RELEASE_RATIO` × 20 = 22 throughout): `massFactor` = 25 / A.mass ≈ 0.9615, so ≈ 0.01444 per tick; cover ends tick 12, seal on tick 35 (progress ≈ 0.5055), B absorbed on tick 70; A mass ≈ 41.99 after payout.                                                                                                                                                                                                                                                                                                                                       |
