...
**refractory** on that prey (`spitOutRefractory = { preyCellId, untilTick = tick + ENGULF_SPIT_OUT_REFRACTORY_SECONDS × TICK_HZ }`):
it cannot start on that prey until the tick after, and the pair is separated meanwhile (§5.3) so the
prey is pushed clear. Only the Diatom Shell sets the chance in build 1 (TRAITS §3.15).
...
3. `phase` = `engulfPhaseOf(progress)` from the progress at the start of the step.
...
| `ENGULF_SPIT_OUT_REFRACTORY_SECONDS`                                   | 1.0       | s                                                                 |
...
- **"Steers away from tick t"** = from tick t every input targets the point 5 radii from the cell's
  ...
  tick t. Placed cells are never pinned in these rows: the predator is idle and stays put, the prey
  moves by the movement step alone.
