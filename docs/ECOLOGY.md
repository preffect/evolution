...
World geometry, session rules and controls: [`GDD.md`](./GDD.md). DNA and levels:
[`PROGRESSION.md`](./PROGRESSION.md). Trait modifiers named below: [`TRAITS.md`](./TRAITS.md).

Units: world units (wu), mass units (mass), seconds (s), ticks at `SIMULATION_TICK_HZ` = 60.
All randomness comes from the seeded `spawner` stream (#73); nothing here uses wall time.
...
| Bacterium     | `BACTERIUM_`    | 3    | 1   | `motile` × 1         | random walk, `BACTERIUM_DRIFT_SPEED`   | 8           | none                        |
...
| ----------------- | ------------------------------------------------------------------------------------------------------------------- | -------------------------------------- | --------------------------------------------- |
| `sunlit_shallows` | Annulus `DISH_RADIUS − SHALLOWS_WIDTH` .. `DISH_RADIUS`                                                             | Light: Chloroplast Pigment works here. | `photic` 0.5, `sensory` 0.5                   |
...
- "players" = cells currently in the dish (connected or in disconnect grace). Caps and rates are
  ...
  cap drops.
- **Kind then zone then point.** A food spawn first draws a kind (`FOOD_KIND_WEIGHTS`: algae 0.75,
  bacterium 0.25), then a zone from that kind's zone weights, then a uniform point inside that zone,
  rejecting points within `FOOD_EDGE_MARGIN` of the wall or inside any cell (retry up to
...
- **Bacteria spawn as clusters** of `BACTERIUM_CLUSTER_SIZE` within `BACTERIUM_CLUSTER_RADIUS` of
  the drawn point; the whole cluster counts toward the cap and consumes one accumulator unit per
  member (the accumulator may go negative and recovers).
...
Applied every tick as `mass −= decayPerSecond × TICK_INTERVAL_S`. Mass never drops below
`CELL_STARTING_MASS` by decay. `zoneDecayMultiplier` is `VENT_DECAY_MULTIPLIER` in the vent, 1 elsewhere.
At mass 1020 in the broth this is 2.0 mass/s: idling halves your surplus in about 6 minutes, so a giant
must keep eating.
...
velocity += (desired − velocity) × CELL_STEER_BLEND_PER_TICK       (= TICK_INTERVAL_S / CELL_ACCELERATION_SECONDS)
...
about a second. `gelSpeedFactor(mass) = clamp(1 − mass / GEL_MASS_SCALE, GEL_MIN_SPEED_FACTOR,
GEL_MAX_SPEED_FACTOR)`: the gel barely slows a starting cell and cuts a 600-mass cell to 40 %, which is
why small cells shelter there. Trait and engulf factors: [`TRAITS.md`](./TRAITS.md), §6.
...
```
requiredRatio = ENGULF_MASS_RATIO + prey.membraneRatioBonus            (Thick Membrane, TRAITS.md)
eligible      = predator.mass ≥ prey.mass × requiredRatio
...
durationS     = ENGULF_BASE_DURATION_SECONDS
                ...
                × prey.engulfDurationMultiplier × predator.engulfDurationMultiplier
```

Engulf is a process, not an event. While `inContact` holds, `progress += TICK_INTERVAL_S / durationS`;
while it does not, `progress −= ENGULF_ESCAPE_DECAY_MULTIPLIER × TICK_INTERVAL_S / durationS`. The
predator's `engulfSpeedFactor` is `ENGULF_PREDATOR_SPEED_FACTOR`, the prey's is
`ENGULF_PREY_SPEED_FACTOR` (membrane grip), both only while progress > 0. Eligibility is rechecked every
tick; losing it releases the prey immediately. A prey is claimed by at most one predator at a time.

**Payout at progress ≥ 1:**
...
| Prey     | state `spectating` for `RESPAWN_SPECTATE_SECONDS`, then respawn ([`GDD.md`](./GDD.md#32-spawn-death-and-respawn)). Keeps level and traits.                                                                                                                     |
| Reserved | trait steal: `ENGULF_TRAIT_STEAL_CHANCE` = 0 in build 1, declared so build 2 can turn it on.                                                                                                                                                                   |

### 6.2 State diagram (per cell)

```
              contact+eligible as prey            progress >= 1
   [free] ------------------------------> [being_engulfed] ----------> [absorbed]
     ^  ^                                      |                            |
     |  |   progress <= 0 or lost eligibility   |                            v
     |  +--------------------------------------+                    [spectating] --3 s--> respawn -> [free]
     |
     |  contact+eligible as predator          prey absorbed / released
     +------------------------------------> [engulfing] -----------------------> [free]
...
| Prey grows past the ratio mid-engulf | Released immediately; the predator keeps no progress.                                                                                                                  |
...
| Partial absorption / nibbling        | Not in v1 (GDD non-goals).                                                                                                                                             |
...
| `CELL_STEER_BLEND_PER_TICK`                                        | 1/15             | derived      |
...
Given seed S and inputs I, after N ticks assert X. Tests place cells and motes through the framework's
fixture helpers where "placed" is stated; everything else comes from the seed.

| #   | Given                                                                                             | Inputs                                              | After      | Assert                                                                                                                                                     |
| --- | ------------------------------------------------------------------------------------------------- | --------------------------------------------------- | ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| E1  | seed 42, 1 player                                                                                 | idle                                                | 0 ticks    | food count = 420, fragment count = 24; every mote within `DISH_RADIUS − FOOD_EDGE_MARGIN`; algae share within 0.75 ± 0.06; no mote inside the player cell. |
| E2  | seed 42, 1 player                                                                                 | idle                                                | 600 ticks  | food count = 490 (7/s × 10 s), fragment count = 30 (0.6/s × 10 s).                                                                                         |
| E3  | seed 42, 1 player                                                                                 | idle                                                | 3000 ticks | food count = 700 (cap); fragment count = 40 (cap).                                                                                                         |
| E4  | seed 42, 1 player, algae mote placed 10 wu east of the centre                                     | idle                                                | 1 tick     | mass = 21, mote gone. Same with a bacterium: mass = 23, DNA = 1, `motile` tag points = 1.                                                                  |
| E5  | seed 42, 1 player placed at mass 1020 in the open broth                                           | idle                                                | 60 ticks   | mass = 20 + 1000 × (1 − 0.002/60)^60 ≈ 1018.00 (± 0.01). Placed in the vent: ≈ 1017.00.                                                                    |
| E6  | seed 42, 1 player placed at mass 320 (then 5000)                                                  | target 5 radii east, 600 ticks                      | 600 ticks  | speed within 0.5 wu/s of 110.0 (then 55.3).                                                                                                                |
| E7  | seed 42, 1 player placed at mass 80                                                               | idle                                                | 1 tick     | radius = 35.78 (± 0.01).                                                                                                                                   |
| E8  | seed 42, 1 player placed at mass 500 at the centre of a gel patch                                 | target 5 radii toward the patch centre              | 600 ticks  | speed within 0.5 wu/s of 49.2 (98.4 × 0.5).                                                                                                                |
| E9  | seed 42, 2 players placed: A mass 100, B mass 20, centres 10 wu apart                             | idle                                                | 30 ticks   | B `absorbed` then `spectating`; A mass = 116, A DNA = 30, A absorptions = 1; detritus motes total mass = 4.                                                |
| E10 | seed 42, A mass 24, B mass 20, centres 10 wu apart                                                | idle                                                | 120 ticks  | no engulf; centre distance ≥ radius(24) + radius(20). With A mass 25: B absorbed at tick 60 (duration 1.0 s).                                              |
| E11 | seed 42, A mass 100, B mass 20, centres 10 wu apart                                               | B: sprint + target 5 radii away from A from tick 10 | 200 ticks  | B `free` by tick 60, alive at tick 200, mass = 20 (sprint cost floored at starting mass); A `free`, absorptions = 0.                                       |
| E12 | seed 42, A placed at mass 5000, algae placed inside A                                             | idle                                                | 1 tick     | A mass = 5000, A DNA = 0.1 (`MASS_OVERFLOW_DNA_PER_MASS`).                                                                                                 |
...
| E14 | seed 42, 1 player, run to `ROUND_BLOOM_START_FRACTION` × round with food held at 0 by the fixture | idle                                                | +600 ticks | food spawned in those 600 ticks = 105 (7 × 1.5 × 10); fragments spawned = 12 (0.6 × 2 × 10).                                                               |
