...
Until #260 reworks `CellModifiers` (TRAITS §2: the `absorb`/`wrap` split and the four new bonuses), the
`Pick<CellModifiers, …>` arguments above are the structurally identical `EngulfPredatorPaceModifiers` /
`EngulfPreyPaceModifiers` declared in `engulf-pace.ts`, filled by the one adapter
`simulation/engulf-modifiers.ts` from the modifiers that exist today. #260 renames the fields and the
adapter becomes a straight `Pick`; no call site changes. `EngulfPaceBalance` and `EngulfRatioBalance`
are `Pick`s of `BalanceConfig['absorption']`
...
expired entries, and entries naming a cell that has left the world, are pruned at the top of the engulf
step itself, which is the only reader): it cannot start on that prey until the tick after, and the pair is
