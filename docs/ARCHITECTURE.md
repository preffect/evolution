...
- **Effects older than the buffer are dropped, not queued.** An effect waits in `pendingEffects`
  ...
  4 after.
- **The window that costs, and the frame rate it costs at.** An effect at tick `T` becomes due when
  the newest snapshot reaches `T + INTERPOLATION_DELAY_TICKS`, and is dropped when it reaches
  `T + SNAPSHOT_BUFFER_SIZE × SNAPSHOT_EVERY_TICKS`. The difference is `EFFECT_DRAW_WINDOW_TICKS`
  (`netcode.ts`), **6 ticks, 100 ms, 10 fps** at the landed cadence — and 2 ticks at 60 Hz, 8 at
  15 Hz, so the floor moves with the cadence rather than being rediscovered. A client drawing slower
  than one frame per window loses effects, in proportion, out of a 300-snapshot stream:

  | frame every | fps   | effects fired of 300 |
  | ----------- | ----- | -------------------- |
  | 1–3 ticks   | 60–20 | 300                  |
  | 4–6 ticks   | 15–10 | 294                  |
  | 8 ticks     | 7.5   | 196                  |
  ...
  Drawing at a frame per snapshot loses nothing; at the window's edge the tick estimate's drift
  costs a couple either side, which is why 10 fps is the floor and not a cliff. Well below it the
  loss is severe and deliberate: a client at 2 fps misses four effects in five and stays alive,
  which is the trade — an eat flash it could not have drawn in time, against the tab surviving.
  `world-store.spec.ts` pins all of it, and each assertion fails when the rule is removed.
