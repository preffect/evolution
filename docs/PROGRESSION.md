...
Ticket: #24. Epic #2. Sources of DNA and tag points: [`ecology/food-and-spawn.md`](./ecology/food-and-spawn.md#1-food-kinds) and
[`ecology/absorption.md §6`](./ecology/absorption.md#6-absorption-and-engulf). The traits being drafted:
[`TRAITS.md`](./TRAITS.md). The ladder the drafts climb: [`game-design/core.md §3`](./game-design/core.md#3-the-evolution-ladder).
Session rules (respawn, round end): [`game-design/session.md §5`](./game-design/session.md#5-session-structure-29).
...
`dnaGainMultiplier` (the Nucleoid Coil, [`traits/catalog-organelles.md §3.1`](./traits/catalog-organelles.md)).

Score for the leaderboard is `dnaCumulative − dnaCatchUpGift + SCORE_ABSORPTION_BONUS × absorptions`
([`game-design/session.md §5.3`](./game-design/session.md#53-leaderboard-and-score)).
...
longer than `ROUND_DURATION_SECONDS` ([`game-design/session.md §5`](./game-design/session.md#5-session-structure-29)).
...
- Design targets for a 10-minute round (the #138 pace model, ± 20 s): solo level 2 at ~2:50, an
  ...
  round length, [`game-design/session.md §5.1`](./game-design/session.md#51-round-timeline-and-pace-curve)).
...
1. **Candidates.** A trait is a candidate when all of these hold:
   ...
   - **the ladder:** the cell has reached the trait's `stage` (`stageOf(ownedTraits)`,
     [`game-design/core.md §3`](./game-design/core.md#3-the-evolution-ladder)), every id in its `requires` is
...
- **Timeout** picks the card with the highest draft weight; ties break by lowest catalog index. The
  ...
  `TRAIT_CHOICE_TIMEOUT_SECONDS` × `TICK_HZ` (shown on tick s, auto-pick applied on tick
  s + `TRAIT_CHOICE_TIMEOUT_SECONDS` × `TICK_HZ` = s + 20 × 60 = s + 1 200).
...
- Applying a card sets the trait to the offered tier, recomputes the cell's modifiers
  ([`traits/model.md §2`](./traits/model.md#2-modifier-model)) and its stage.
...
([`ecology/food-and-spawn.md §3.1`](./ecology/food-and-spawn.md#31-the-world-clock), `worldReference` at the entry tick) is the
...
| Field           | Rule                                                                                                                                                                                                                                                                                                                                                                        |
| --------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `dnaCumulative` | raised to `max(current, floor(ENTRY_DNA_FRACTION × median dnaCumulative), worldDna)`; the raise is added to `dnaCatchUpGift` (score-neutral, [`game-design/session.md §5.3`](./game-design/session.md#53-leaderboard-and-score)). The median term exists only for a late join after `ENTRY_GRACE_SECONDS` with another living player; a respawn uses the world floor alone. |
...
- **Respawn.** Keeps level, traits and stage, keeps only `dnaKeptOnDeathFraction` of
  `dnaTowardNextLevel` ([`game-design/session.md §5.2`](./game-design/session.md#52-spawn-death-and-respawn)), then
...
- **Wild cells** ([`ecology/wild-cells.md §3.3`](./ecology/wild-cells.md#33-wild-cells)) do not use this rule: their ladder
  is the world's every tick and each is born at its base size for that moment (§3.3.5), so they need no floor.
...
| `TRAIT_CHOICE_TIMEOUT_SECONDS`   | 20                                 | s       |
...
live in `ladder.ts` ([`game-design/constants-and-acceptance.md §12`](./game-design/constants-and-acceptance.md#12-constants-table)).
...
Conventions as in [`ecology/acceptance.md §8`](./ecology/acceptance.md#8-acceptance-scenarios). Under decision #138
...
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------- | ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| P1  | seed 42, 1 player (seeded world)                                                                                                                                                                                                                                    | greedy bot                       | 21 600 ticks  | level ≥ 2 (the ticket #75 example scenario; 6:00 = twice the ecology/food-and-spawn.md §3 solo estimate of ~2:50).                                                                                                                                                                                                                                                        |
...
| P3  | P2 state (offer shown on tick 12)                                                                                                                                                                                                                                   | no `traitChoice` for the timeout | timeout ticks | the timeout is `TRAIT_CHOICE_TIMEOUT_SECONDS` × `TICK_HZ` = 20 × 60 = 1 200 ticks: offer closed on tick 12 + 1 200 = 1 212 exactly (boundary rule, §4), still open after 1 211; `nucleoid` owned at tier I (weights 1.0 / 0.5 / 1.0: none of the three picks carries `sensory`, so the twelve tag points change nothing; tie → lowest catalog index); stage `prokaryote`. |
...
| P10 | seed 42, 1 player at level 12 with fixture DNA 1760 (fixture only: under #138 option A no player reaches level 12 in a 600 s round, game-design/session.md §5.1)                                                                                                    | eat 1 placed DNA fragment        | 1 tick        | `dnaCumulative` = 1765, level 12, no offer.                                                                                                                                                                                                                                                                                                                               |
...
| P13 | pure function `stageOf`                                                                                                                                                                                                                                             | —                                | —             | [] → `protocell`; [nucleoid] → `prokaryote`; [nucleoid, chloroplast] → `endosymbiosis`; + nuclear_envelope → `eukaryote`; + euglena_eyespot → `specialised`; [chloroplast] alone → `endosymbiosis` (the highest gate owned; only a fixture can grant a gate out of order, the draft never offers one).                                                                    |
