...
| P5  | ECOLOGY E9 with A given `dnaCumulative` = `dnaTowardNextLevel` = 40 at setup (A level 1 absorbs B on tick 36; B has no DNA, so the gain is `ENGULF_DNA_BASE` = 30)                                                                                                  | idle                           | 31 ticks     | A `dnaCumulative` = 70, level = 2, `dnaTowardNextLevel` = 10 (70 − 60), exactly one offer shown (opened on tick 30). Without the 40 banked: `dnaCumulative` = 30, level 1, no offer.                                                                                                   |
