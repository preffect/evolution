...
- **The world climbs the same ladder.** The world clock ([`ECOLOGY.md §3.1`](./ECOLOGY.md#31-the-world-clock))
  gives the dish itself a `worldStage` from this enum, one rung per world level (protocell 0:00,
  prokaryote 3:00, endosymbiosis 6:00, eukaryote 9:00 in a 600 s round); the wild cells wear that
  stage's organelles, and a player's standing is read against it (section 5.5). Late joiners and
  respawns enter the ladder no lower than the world's rung (section 5.2, [`PROGRESSION.md §5`](./PROGRESSION.md#5-entering-the-dish-late-join-and-respawn)).
...
| Late join           | Allowed at any time; the joiner enters at the world's rung at least ([`PROGRESSION.md §5`](./PROGRESSION.md#5-entering-the-dish-late-join-and-respawn)). |
| Death               | Engulfed cell spectates `RESPAWN_SPECTATE_SECONDS` = 3, then respawns at the world's rung at least (section 5.2).                                        |
...
- **Death** = being fully engulfed ([`ECOLOGY.md`](./ECOLOGY.md#6-absorption-and-engulf)), by a
  ...
  the world's rung is lifted to it (a score-neutral gift, drafts queued), a player at or above it
  keeps its own, and the respawn mass is `ENTRY_MASS_FRACTION` of the world's mass clamped to
  [`CELL_STARTING_MASS`, `ENTRY_MAX_MASS`] (20 in the first minutes, 0.25 × 410 ≈ 102 at 6:30, 140 at 9:00). The
...
- **What it drives:** the mote mix (more bacteria as the world ages), the broth's share of
  ...
  endosymbiosis era.
- **Entering the dish** (late join and respawn) floors the player at the world's rung
...
wild cells that eat, grow or draft (they are the world clock made flesh, [`ECOLOGY.md §3.3`](./ECOLOGY.md#33-wild-cells);
NPC microbes with their own progression are not in build 1), trait stealing, partial absorption
...
| G8  | ECOLOGY E9 setup (A absorbs B at tick 30)                                                                                                                                                                                       | idle                                  | 31 ticks     | leaderboard[0] = A with score = A.dnaCumulative + 25 = 55; B `lifeState` = `'spectating'`, `spectatingCellId` = A's cell, no cell for B; at tick 211 (30 + 180 + 1) B alive at starting mass (entry rule: 0.25 × `worldMass`(3.52 s) = 5.88 < 20), level, traits and stage unchanged (world level 1.02: no lift).                                                                                                                        |
...
| G11 | seed 42, 1 player                                                                                                                                                                                                               | idle                                  | 37 200 ticks | `world_level_up` effects on ticks 10 800 (`{ level: 2, stage: 'prokaryote' }`), 21 600 (3, `endosymbiosis`) and 32 400 (4, `eukaryote`) and on no other tick; through `results` (ticks 36 000–37 199) `worldReference` stays at 600 s (level 4.33); at tick 37 200 (rematch, seed 43) elapsed is 0: world level 1, every wild cell a protocell at mass 20 × its spread.                                                                  |
...
| G13 | seed 42, 1 player A placed at level 1, mass 20 (no DNA); the fixture absorbs A's cell after tick 23 400 (6:30)                                                                                                                  | idle                                  | 23 581 ticks | A alive on tick 23 581 (23 400 + 180 + 1); elapsed 393.017 s → world level 3.18 → A `dnaCumulative` = 140 = `dnaCatchUpGift`, level 3, `dnaTowardNextLevel` = 0, one offer shown (the protocell draft) and one queued, score 0, no tag points; mass = 0.25 × 413.017 ≈ 103.25 (± 0.01; respawn runs at step 9, after this tick's decay). With A at `dnaCumulative` 200 (level 3) at setup instead: gift 0, level 3, still mass ≈ 103.25. |
| G14 | seed 42, 1 player idle; a second player joins before tick 18 000 steps (5:00)                                                                                                                                                   | idle                                  | join + 1     | median term 0 (the living player has no DNA) but the world floor is level floor(2.667) = 2: joiner `dnaCumulative` = 60 = `dnaCatchUpGift`, level 2, one offer shown; mass = 0.25 × max(20, 320) = 80, then one tick of decay ≈ 80.00 (± 0.01); score 0. PROGRESSION P7 and P8 hold unchanged (their world floor is level 1).                                                                                                            |
