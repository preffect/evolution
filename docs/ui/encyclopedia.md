...
| `format/navigation.ts`, `format/search.ts`, `format/panel-keys.ts`, `format/rail-view.ts`, `format/list-view.ts`, `format/landing-view.ts`, `format/panel-view.ts`, `format/glyph-subject.ts`, `format/entry-view.ts` | Pure: the transitions, the match and its order, the panel's key rules, the page's view model (tier columns, chips, crumbs)                                |
