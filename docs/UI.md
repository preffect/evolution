...
or 135. The ghost keeps its angle. A counter that would come nearer to it than `LADDER_ITEM_CLEARANCE_PX` turns
away (aerobic clockwise, photosynthetic counter-clockwise) until it is exactly that clear. On its §9 angle the
counter's gap to the ghost is `π/4 × ladderOrbitRadiusPx − 32` px (half a ghost plus half a counter), so the turn
happens below 31 px (the 24 px spawn) and never from 31 px up. Where the padded backings meet, `orbitLayout`
merges them into one band with no darker seam: at 24 and 32 px; they stand apart at 45 px.
...
| `LADDER_ITEM_CLEARANCE_PX`                                  | 4          | px   | A rung ghost to a counter sharing the orbit (#285 B): the counter turns away until it is this clear.                  |
