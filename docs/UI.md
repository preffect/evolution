...
custom property (`hud/format/hud-css-variables.ts`, unit-tested against the constants) and the stylesheets read
`var(--hud-…)`, so the value still has exactly one home. The type scale and the two font stacks are
