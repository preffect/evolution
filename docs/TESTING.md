...
| `client` | `testing/fake-websocket.ts`, `testing/fake-audio-backend.ts`, `testing/fake-audio-context.ts`, `testing/fake-bake-canvas.ts`, `testing/builders.ts` | `FakeWebSocket`: install with `vi.stubGlobal('WebSocket', FakeWebSocket)`, then `open()` / `receive()` / `close()` from the test; `FakeAudioBackend` (records buses, decodes and voices, `shouldThrow`), `FakeAudioAssetLoader`, `MemoryMutePreference`; `FakeAudioContext` for `vi.stubGlobal('AudioContext', …)`; `FakeBakeContext` / `createFakeBakeCanvasFactory` (a recording Canvas 2D for the texture bakes, #99); `createTestCellView`, the effect builders, `createTestTransitionOptions`, `createTestAudioManifest` |
