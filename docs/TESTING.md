...
| `client` | `testing/fake-websocket.ts`, `testing/fake-audio-backend.ts`, `testing/fake-audio-context.ts`, `testing/fake-pixi-app.ts`, `testing/builders.ts` | `FakeWebSocket`: install with `vi.stubGlobal('WebSocket', FakeWebSocket)`, then `open()` / `receive()` / `close()` from the test; `FakeAudioBackend` (records buses, decodes and voices, `shouldThrow`), `FakeAudioAssetLoader`, `MemoryMutePreference`; `FakeAudioContext` for `vi.stubGlobal('AudioContext', …)`; `createFakePixiApp` / `createFakeTextureBaker` (a `PixiAppHandle` with no WebGL: a real stage, a hand-run ticker, a recording baker, #205); `createTestCellView`, `createTestRenderFrame`, the effect builders, `createTestTransitionOptions`, `createTestAudioManifest` |
