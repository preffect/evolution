...
| `client` | `testing/fake-websocket.ts`, `testing/fake-audio-backend.ts`, `testing/fake-audio-context.ts`, `testing/fake-pixi-app.ts`, `testing/fake-bake-canvas.ts`, `testing/builders.ts` | `FakeWebSocket`: install with `vi.stubGlobal('WebSocket', FakeWebSocket)`, then `open()` / `receive()` / `close()` from the test; `FakeAudioBackend` (records buses, decodes and voices, `shouldThrow`), `FakeAudioAssetLoader`, `MemoryMutePreference`; `FakeAudioContext` for `vi.stubGlobal('AudioContext', …)`; `createFakePixiApp` / `createFakeTextureBaker` / `createTestRenderTextures` (a `PixiAppHandle` with no WebGL: a real stage, a hand-run ticker, a baker that records radial specs and canvas bakes and hands back 1×1 textures, #205, #206); `testing/fake-bake-canvas.ts`: `FakeBakeContext` / `createFakeBakeCanvasFactory` / `fakeContextOf` (a recording Canvas 2D: ops, gradients and stops, `paintCount`, #206); `createTestCellView`, `createTestRenderFrame`, the effect builders, `createTestTransitionOptions`, `createTestAudioManifest` |
