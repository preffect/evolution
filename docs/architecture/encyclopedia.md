...
- **The engulf phases.** `ENGULF_COVER_SECONDS`, `ENGULF_WRAP_SECONDS` and `ENGULF_ABSORB_SECONDS` are the balance
  leaves; the base duration and the two progress bands are derived from them at read time (#367,
  architecture/constants-files-tests.md §9), and the simulation reads them the same way. The phase spans shown and
  previewed are `engulfPhaseSpanSeconds(phase, balance.absorption)` = the base duration × the phase's progress band
  (cover `[0, wrapStart)`, wrap `[wrapStart, seal)`, absorb `[seal, 1]`), each its phase second up to float rounding,
  in shared `simulation/engulf-pace.ts` beside `engulfPhaseOf`, so a patched phase second moves the shown spans exactly
  as it moves the engulf.
...
/**
 * What draws each page's element. Five of the seven are drawn by the renderer on the own cell and share the one
 * `ownCell` status mirror, so a `HUD_TEST_ID` key alone would anchor none of them: those name their field of
 * `OwnCellIndicators` (`state/own-cell-indicators.ts`, a type-only import). `HUD_TEST_ID` lives in the neutral
 * `game/test-ids/hud-test-ids.ts` (which `hud/` and `input/` import too), so the anchor keeps "encyclopedia/ never
 * imports hud/", type imports included. The completeness spec pins that the seven anchors are pairwise distinct.
 */
export type HudElementAnchor =
  | { readonly kind: 'dom'; readonly testId: keyof typeof HUD_TEST_ID } // leaderboard, round_clock
  | { readonly kind: 'indicator'; readonly field: keyof OwnCellIndicators }; // the DNA ring, level numeral, ladder orbit, self ring, threat ring
...
- **Unit:** §12.6's specs; `engulf-pace.test.ts` in shared (`engulfPhaseSpanSeconds` sums to
  `ENGULF_BASE_DURATION_SECONDS` over the three phases and follows a patched `ENGULF_WRAP_SECONDS`);
  ...
  `scenes/engulf-scenes.spec.ts` (the engulf scene's phase boundaries move with a patched
  `ENGULF_WRAP_SECONDS` and `ENGULF_COVER_SECONDS`, the leaves the simulation reads, #367; the absorbed prey
