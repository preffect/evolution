...
// encyclopedia/model/actions.ts — every player intent on the wire and every effect has an entry
export const ACTION = {
  steer: 'steer',
  sprint: 'sprint',
  eat: 'eat',
...
facts and headings. Keeping each entry's copy in one object is also what a later localisation replaces.
...
- `packages/client/src/app/game/encyclopedia/content/**`: `no-restricted-syntax` rejects number literals and binary
  arithmetic, so a content file cannot hold a number or compute one even through a named constant.
...
`render/constants/preview.ts` constant), the fixture gel patches and the session's `devicePixelRatio`; `show` never
...
- **Canvas:** the lens's bounding square, `sizePx = { width: D, height: D }` with `D = ENCYCLOPEDIA_LENS_DIAMETER_PX × --ui-scale`, so
  ...
  the preview caps its DPR at `PREVIEW_MAX_DEVICE_PIXEL_RATIO` 2, so its backbuffer stays near 3 MiB at worst.
- **Crop:** a CSS circle on the stage element that holds the canvas (`clip-path: circle(50%)`), never a Pixi mask:
  the compositor clips for free, where a stencil mask would add draw calls and a stencil pass every frame. The only
  waste is the square's corners (1 − π/4 ≈ 21 % of the fill). The loading and unavailable states fill the same
  clipped circle, so no square shows before the first frame. The renderer's own screen vignette is square; its
  corners fall outside the clip.
...
the clips already in progress exactly as a live preview does at `t`. It clips the stage exactly as the encyclopedia does, so graphics-qa's element screenshots show the lens the player sees, while the pixel-equality checks read the canvas buffer (`toDataURL`, the full square: a CSS clip does not touch it). It is the one place the preview installs
