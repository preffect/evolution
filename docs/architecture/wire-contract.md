...
- **`join_game` / `create_game` while seated in another room** (#334): two tabs share one `clientId`, so a socket
  can take a seat elsewhere without a `leave_game`. Once the server accepts the frame (a `join_game` for a game that
  exists and has a free seat; a `create_game` always), it first leaves the old seat exactly as `leave_game` does,
