...
`game_state` for the same player and round length (a resync) keeps the live session and only takes the balance, so
a client that is resynced keeps its loops and its transition memory; another player or round length starts over.
