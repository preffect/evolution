...
cells/forms/amoeba-pseudopods.ts                   the amoeba's core, its lobes' reach, fan and cycle, and their peak bound (#192)
...
- **Unit (vitest, no WebGL):** `radial-profile.spec.ts` pins `r(θ)` at 36 angles per state against literal
  ...
  cycle within 0.5 %, the fan gathering with speed and aiming at the prey, the staggered cycle, a peak no frame beats and some frame reaches);
  `cell-draw-extent.spec.ts` walks every form's drawn membrane against the body bound; `organelle-layout.spec.ts` (slot centres inside 0.92 and outside `DNA_RING_KEEP_OUT_FRACTION`, every sprite body inside the membrane, the
