...
| `simulation.ts`, `netcode.ts`                                     | `ARCHITECTURE.md §3, §5`             | engineering constants that are not gameplay tunables: `SPATIAL_HASH_CELL_SIZE_WU`, `MAX_TICKS_PER_ADVANCE`, `SNAPSHOT_EVERY_TICKS`, `SNAPSHOT_POSITION_DECIMALS`, `SNAPSHOT_BUFFER_SIZE`, `INTEREST_MARGIN_WU`, `MAX_EXTRAPOLATION_TICKS`, `RECONCILE_SNAP_DISTANCE_WU`, `RECONCILE_BLEND_SECONDS`, `INTERPOLATION_DELAY_TICKS`, `REPLAY_FORMAT_VERSION`; excluded from `balance.json` |
...
| File                                                     | Exempt from                      |
| -------------------------------------------------------- | -------------------------------- |
| `packages/server/src/lobby/lobby-manager.ts` (338 lines) | `max-lines`, timers              |
