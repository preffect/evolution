...
design docs ([`game-design/constants-and-acceptance.md §12`](./game-design/constants-and-acceptance.md#12-constants-table) and companions).
...
(`packages/shared/src/constants/**`, the client's `render/constants.ts` and its pages under `render/constants/`) and in tests and
...
| ----------------------------------------------------------------------------------- | ----------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `units.ts`, `network.ts`, `lobby.ts`, `identity.ts`, `debug.ts`                     | the template (already split in #112)                              | `MILLISECONDS_PER_SECOND`, `BYTES_PER_MEBIBYTE`, `NANOSECONDS_PER_MILLISECOND` (GL timer queries), `P95_QUANTILE` (the one quantile of the server tick stats and the client frame report); `TICK_HZ`, `TICK_INTERVAL_MS`, `DISCONNECT_GRACE_MS`, ports, WebSocket deflate settings; name/avatar/player bounds, `GAME_ID_LENGTH`, `DEFAULT_PLAYERS_PER_GAME`, `SEAT_MARK_BEADS` (owner `visual-style/principles-and-palette.md` §2; shared because the HUD and the renderer both read it); storage key; `DEBUG_JSON_INDENT_SPACES`, `DEBUG_STEP_MAX_SECONDS` (seconds; `secondsToTicks` at the tool's schema, never a ticks twin in `constants/`), `DEFAULT_BOT_SEED`                                                                                                                                                                                                                          |
| `world.ts`, `session.ts`, `controls.ts`, `ladder.ts`, `camera.ts`, `world-clock.ts` | `game-design/constants-and-acceptance.md §12`                     | dish, spawn safety, round, seed bound, sprint, `STAGE_ORDER`, camera, the world clock (`WORLD_LEVEL_SECONDS`, `WORLD_MASS_GAIN_PER_SECOND`, `WORLD_STANDING_MASS_TOLERANCE`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `ecology.ts`, `growth.ts`, `absorption.ts`, `wild-cells.ts`                         | `ecology/constants.md §7`                                         | food kinds, spawners (by world stage), zones, decay, curves, engulf, wild cells (`WILD_CELL_*`, `WORLD_ORGANISM_ID`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `progression.ts`                                                                    | `PROGRESSION.md §6`                                               | levels, draft weights, timeout, entry rule                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `traits.ts`                                                                         | `traits/constants-and-acceptance.md §5`                           | `TRAIT_CATALOG`, `TRAIT_TIERS` (derived from the catalog), `RESERVED_TRAIT_IDS`, `EXCLUSION_GROUPS`; the numbers themselves (`DEFAULT_CELL_MODIFIERS`, one `<ID>_TIERS` table per trait) sit in `trait-modifiers.ts` and are re-exported, so `traits` stays one domain                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `balance.ts`                                                                        | `architecture/constants-files-tests.md §9`                        | `DEFAULT_BALANCE` (the domain modules spread into plain records, deep-frozen), `BalanceConfig`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `motion.ts`                                                                         | `rendering/contents-and-motion.md §4`                             | the sheet-03 motion clips (keyframes, easing names, durations): cosmetic data read by the renderer, the HUD and the sound bus; never by the server; excluded from `balance.json`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `audio.ts`                                                                          | `AUDIO.md §2, §3`                                                 | `SOUND_EVENT_CATALOG` (priority, cooldown, loop, bus per `SoundEventId`), `SOUND_PRIORITY`, `MAX_OVERLAPPING_CUES`, the stem / motif layering tables, crossfade and duck numbers, `AUDIO_MUTE_STORAGE_KEY`, the manifest path and version: cosmetic data read by the client's audio layer only, never by the server; excluded from `balance.json` like `motion.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `simulation.ts`, `netcode.ts`, `derive-netcode.ts`                                  | `architecture/server-simulation.md §3, architecture/client.md §5` | engineering constants that are not gameplay tunables: `SPATIAL_HASH_CELL_SIZE_WU`, `MAX_TICKS_PER_ADVANCE`, `FIXED_STEP_ROUNDING_TOLERANCE_TICKS`, `SPAWN_ACCUMULATOR_TOLERANCE`, `GEL_PATCH_PLACEMENT_MAX_ATTEMPTS`, `INITIAL_FILL_POINT_MAX_ATTEMPTS`, `SNAPSHOT_EVERY_TICKS`, `SNAPSHOT_ACK_INTERVAL_TICKS`, `SNAPSHOT_ACK_EVERY_SNAPSHOTS`, `SNAPSHOT_ACK_EVERY_TICKS`, `EFFECT_DRAW_WINDOW_TICKS`, `SNAPSHOT_POSITION_DECIMALS`, `SNAPSHOT_RADIUS_DECIMALS`, `SNAPSHOT_VELOCITY_DECIMALS`, `SNAPSHOT_MASS_DECIMALS`, `SNAPSHOT_SCORE_DECIMALS`, `SNAPSHOT_BUFFER_SIZE`, `MAX_EXTRAPOLATION_TICKS`, `RECONCILE_SNAP_DISTANCE_WU`, `RECONCILE_BLEND_SECONDS`, `MAX_PREDICTION_TICKS`, `PREDICTION_STALL_TICKS`, `INTERPOLATION_DELAY_TICKS`, `INTERPOLATION_DELAY_INTERVALS`, `SERVER_TICK_ESTIMATE_SMOOTHING`, `SNAPSHOT_STALE_MS`, `REPLAY_FORMAT_VERSION`; excluded from `balance.json` |
| `interest.ts`                                                                       | `architecture/wire-contract.md §4.2 lever 1`                      | viewport culling, the parts that do not depend on the balance: `INTEREST_VIEW_ASPECT_RATIO`, `INTEREST_CAMERA_HISTORY_BROADCASTS`, `INTEREST_ENTITY_REACH_RADII`. The margin itself is a formula over the room's live balance, `interestMarginFor(balance)` in `camera/interest-margin.ts`, since `debug_set_balance` can raise the speeds it bounds; excluded from `balance.json`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| Ids and kinds (message verbs, `EntityKind`, `TraitId`, `CellStage`, `DnaTag`, `BacteriumVariant`, `SoundEventId`)       | `types/messages.ts` (verbs), `types/game.ts`, `types/traits.ts` and `types/audio.ts` (`SOUND_EVENT`, `AUDIO_BUS`) as `as const` objects with the union derived from each (`ValueOf`, `types/common.ts`); the ordered arrays (`STAGE_ORDER`, `DNA_TAGS`, `BACTERIUM_VARIANTS`) fix the walk order and are pinned complete against the objects                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | object `UPPER_SNAKE_CASE`, keys camelCase, values `snake_case`                           |
| Random stream labels                                                                                                    | `packages/shared/src/random/stream-labels.ts` (`RANDOM_STREAM`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | values `snake_case`                                                                      |
| Client render-only numbers (colours, layer z, wobble amplitude, granule count)                                          | `packages/client/src/app/game/render/constants.ts` only (a barrel over the pages `render/constants/{colours,ui-type,own-cell,cell-shape,organelles,vent,world-render,bench,preview}.ts`, so every import still reads `render/constants`); camera numbers are gameplay-visible and live in `camera.ts`; the frame budgets and the bench load (`rendering/budget.md §6–§7`) are the `bench` page, ledgered against those tables by `render/bench/render-budget-ledger.spec.ts`; the encyclopedia preview's framing, seed and budgets (`architecture/encyclopedia.md §12.7`) are the `preview` page; plus the own-cell indicators' reading floors, pill sizes and alphas, whose values are `ui/components-and-constants.md §9`'s and are ledgered by the client's `render/constants-ledger.spec.ts` against that table | `UPPER_SNAKE_CASE`, unit suffix                                                          |
| Client input key codes, direction vectors and the DOM selectors the hotkey rules read (`ui/input-and-onboarding.md §4`) | `packages/client/src/app/game/input/input-constants.ts` only; the steering and sprint tunables stay in `constants/controls.ts` and reach the client through the live `game_state.balance`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | `UPPER_SNAKE_CASE`, unit suffix                                                          |
| Client HUD layout numbers and hint timings (`ui/layout.md §1`)                                                          | `packages/client/src/app/game/hud/hud-constants.ts` only                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | `UPPER_SNAKE_CASE`, unit suffix                                                          |
| Player-facing units: unit words, suffixes, decimals, sign rules (`architecture/encyclopedia.md §12.5`)                  | `packages/client/src/app/game/quantities/quantity-unit.ts` only (with `PERCENT` and the tier numerals); every number the player reads as text in the HUD, the menu or the encyclopedia is formatted by `quantities/format-quantity.ts`, and the modifier label table is `quantities/modifier-labels.ts`. `data-*` attribute values are test hooks, not text, and stay raw. Encyclopedia content types no number and no arithmetic: its facts read the live balance through `encyclopedia/facts/` (§12.3)                                                                                                                                                                                                                                                                                                            | `QUANTITY_UNIT` object `UPPER_SNAKE_CASE`, values `snake_case`                           |
...
- **Derived values are computed, never declared** (`game-design/constants-and-acceptance.md §12`): `TICK_INTERVAL_MS`
...
- **Open/closed by data.** Trait effects are the `TRAIT_TIERS` tables folded by
  `foldModifiers` into one `CellModifiers` record (traits/model.md §2); food kinds, bacterium variants,
...
that used to be in `engineering/conventions-and-done.md` §4.6.
...
Prefer pure functions and plain data; classes for genuinely stateful things (`engineering/conventions-and-done.md`
...
- **Throw on impossible state, never on player input.** Missing required config, an unknown
  ...
  The reserved `shouldSplit` / `shouldEject` flags pass the schema and are ignored without counting
  (`architecture/server-simulation.md §3.2`).
...
- **Client:** `AudioService` and the renderer degrade silently by design (documented in
  `architecture/client.md` §7); everything else surfaces through the HUD's error banner.
...
- Co-located: `foo.ts` → `foo.test.ts` (client `foo.spec.ts`), never a `__tests__/` directory; integration
  `foo.integration.test.ts` (`engineering/testing-and-typescript.md` §2.2). Gameplay scenarios in
  `packages/server/src/testing/scenarios/<table>.gameplay.test.ts`, one file per design table,
  tests named by row id (`E9`, `P3`, `T4`, `G2`), on the runner of `testing/scenario-runner.md` §8 (#75, #102).
- Builders, not fixtures files: `createTestCell`, `createTestWorld`, `createTestSnapshot` in
  each package's `src/testing/builders.ts` (`testing/tiers-and-builders.md` §4 lists what exists today); defaults
...
- Assert values and hashes, not object identity and not snapshots of large objects
  (`architecture/server-simulation.md §3.1`: the simulation mutates in place; `expect(result).not.toBe(prev)`
  is for reducers that return new state).
- Seed every random test (`createSeededRandom(TEST_SEED)`); a test that uses time uses
  `ManualClock`. Scenario arithmetic follows the fixture conventions of `ecology/acceptance.md §8`.
