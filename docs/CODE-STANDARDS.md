...
| Ids and kinds (message verbs, `EntityKind`, `TraitId`, `CellStage`, `DnaTag`, `BacteriumVariant`, `SoundEventId`) | `types/messages.ts` (verbs), `types/game.ts` and `types/traits.ts` as `as const` objects with the union derived from each (`ValueOf`, `types/common.ts`); the ordered arrays (`STAGE_ORDER`, `DNA_TAGS`, `BACTERIUM_VARIANTS`) fix the walk order and are pinned complete against the objects                  | object `UPPER_SNAKE_CASE`, keys camelCase, values `snake_case`                           |
