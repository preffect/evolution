...
| Client render-only numbers (colours, layer z, wobble amplitude, granule count)                                    | `packages/client/src/app/game/render/constants.ts` only (a barrel over the pages `render/constants/{colours,cell-shape,organelles,world-render}.ts`, so every import still reads `render/constants`); camera numbers are gameplay-visible and live in `camera.ts`; plus the own-cell indicators' reading floors, pill sizes and alphas, whose values are `UI.md §9`'s and are ledgered by the client's `render/constants-ledger.spec.ts` against that table | `UPPER_SNAKE_CASE`, unit suffix                                                          |
