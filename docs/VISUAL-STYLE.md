...
| Engulf (cell)            | sheet 03 strip B, **driven by `engulfProgress`, not the clock**: contact → wrap over 0 → 0.5, seal over 0.5 → 1.0 (arms and notch are functions of progress, so a progress decay plays backwards); dissolve → DNA streams → done is a 600 ms effect at payout. Sheet 03's "the escape window closes at the seal frame" is superseded: escape is possible until progress reaches 1, at 2× decay (`ECOLOGY.md §6.1`)                                                                                                                          | sheet 03 deformation values   |
