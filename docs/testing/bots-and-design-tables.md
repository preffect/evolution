...
player leaving mid-engulf on either side (ecology/absorption.md §6.3), two bots charging each other (§5.3 separation; the
head-on pass-through is an `it.fails` row until ticket #709 lands) and a seeded three-minute playthrough of two `hunter` and two
