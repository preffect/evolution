...
- **Tickets and briefs cite spec files, not documents.** The design specs are topic files under
  ...
  (generated by `scripts/docs-index.sh`, checked by `./validate.sh lint`) lists every heading of every doc with
  its line range, for citing a range inside a file; ingesting whole documents to find one section is the main
