...
- A worktree per branch (`.worktrees/<branch>`, `scripts/worktree.sh`) lets several agents run in
  ...
  stops the run).
- **Briefs cite sections, not documents.** `docs/INDEX.md` (generated by `scripts/docs-index.sh`,
  checked by `./validate.sh lint`) lists every heading of every doc with its line range. A brief says
  `ECOLOGY.md §7 (L540–610)` and the agent reads that range; ingesting whole documents to find one
  section is the main way an agent's context grows before it has done anything.
