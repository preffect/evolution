...
| trait halo                    | flat to 0.86, → 0 at 1.39, replaces the halo                                                                                                                                                                                                                                                                | `CHLORO_LIGHT` / `TOXIN_GLOW` @42 % (sheet 01 trait legibility; `halo-chloro` has the same 0.62 stop)                                                                                                                                                                                                                                                                                    | A    | ≥ mid                      |
...
| cilia                         | 1.00 → 1.12, **leaning hairs** of constant px width: `lean = d / r · tan(CILIA_LEAN_DEG 30°)`, `θ_h = θ − lean + wave`, `wave = CILIA_WAVE_AMPLITUDE_RAD · sin(2π θ · CILIA_WAVE_COUNT − ω t)`, `s = fract(N θ_h / 2π)`, mask `abs(s − 0.5) · 2π ‖p‖ / N < CILIA_WIDTH_PX / 2` (1.2 px, fwidth-antialiased) | `CILIA` @75 % × `(1 − d / 0.12 r)` (fade to the tip); N 24 / 36 / 48; ω ∝ speed; hairs stay rooted, the beat is a travelling wave of the lean (sheet 04 fringe: 1.2 px lines leaning 30°, metachronal wave). `CILIA_WAVE_COUNT` 3, `CILIA_WAVE_AMPLITUDE_RAD` 0.15 are new. Mid: flat band @40 %                                                                                         | B    | ≥ mid                      |
