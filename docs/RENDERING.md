...
| contact     | bump −0.12 × depth, σ 22° toward the neighbour; σ 14° with `cytoskeleton`; depth = overlap / (`CONTACT_DENT_FULL_OVERLAP_RADII` 0.25 × r_min) clamped, so the dimple grows with the press and eases out with the separation | VISUAL-STYLE §5                                                                                                                                                                                                         | `cells/contact-dents.ts` (visible-cell scan)  |
...
| ribosome speckle              | 0.55 → 0.89, **hashed dot grid** in the undeformed frame: pitch `sqrt(annulusArea / density)` = 0.28 / 0.20 / 0.16 r, one dot per grid cell at `hash(speckleSeed, i, j)` offset (`speckleSeed`: one draw from the cell's cosmetic fork, §2.3, so two cells of one palette never share a lattice, #243), drawn `1 − smoothstep(rad − fw, rad + fw, dist)`                                                | `RIBOSOME`; radius hashed in `RIBOSOME_RADIUS_RADII` 1.2–2.2 % r with the **diameter** floored at `RIBOSOME_MIN_PX` 2 px, the offset hashed across `RIBOSOME_JITTER_SHARE` 0.8 of the pitch independent of the radius (so play-scale dots stay a stipple, never a lattice), alpha hashed in `RIBOSOME_ALPHA` 0.35–0.80; density 20 / 40 / 60 (VISUAL-STYLE §4; sheet 01 panel A: 28 crisp circles r 1.6–2.8 px @0.26–0.60). Same grid, other band, serves protocell granules if they ever leave sprites | A    | full                              |
...
- **Floors.** `dnaRingRadiusPx` and `ladderOrbitRadiusPx` (`effects/own-cell-geometry.ts`) and `orbitLayout` (`effects/orbit-layout.ts`), all pure, apply UI.md
  ...
  pips after, clockwise, turns a counter away from a rung ghost it would crowd (UI.md §3.1.3; the gap is measured between the drawn,
