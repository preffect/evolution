...
- **Evolution — UI: HUD, overlays and onboarding: input, onboarding and readability** (L1–155): §4–§6 of the split `UI.md`, which keeps the shared context and the file list.
...
- **5. Onboarding: the first two minutes** (L75–113): Diegetic and text hints, no modal tutorial.
- **6. Readability during play** (L114–155): and the connection banner stack from the top, never downward past `HUD_NOTICE_STACK_BOTTOM_PX` (y 96).

## ui/layout.md (102 lines)

- **Evolution — UI: HUD, overlays and onboarding: layout frame and lobby screens** (L1–102): §1–§2 of the split `UI.md`, which keeps the shared context and the file list.
...
- **2. Screens (lobby)** (L91–102): The join flow is the template's (#100: "lobby tagline and join flow unchanged"), with the game's config fields and stable test ids added.

## ui/overlays.md (204 lines)

- **Evolution — UI: HUD, overlays and onboarding: trait pick, death, results, menu and notices** (L1–204): §3.2–§3.7 of the split `UI.md`, which keeps the shared context and the file list.
  ...
  - **3.5 Menu (Escape)** (L103–116): Escape closes the topmost open overlay (`openOverlay = 'leaderboard'`) and, with none open, sets `openOverlay = 'menu'`: a 320-wide centred…
  - **3.6 Notices: toasts and connection states** (L117–148): One toast at a time, top-centre at y 16, `body` on the callout backing, `TOAST_DURATION_SECONDS`, newest replaces oldest, `aria-live="polit…
  - **3.7 Hold-Tab "affecting you" panel (`openOverlay === 'leaderboard'` and `lifeState === 'alive'`)** (L149–204): Decision #324 (option C).
...
- **Evolution — Visual Style: UI colours and type** (L1–33): §7 of the split `VISUAL-STYLE.md`, which keeps the shared context and the file list.
- **7. UI colours and type** (L5–33): Panels, text, chips and bars use sheet 03's palette table and the HUD / trait-picker layouts.
