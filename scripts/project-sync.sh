...
#
#   1. Every open or closed issue in the repo is on the project (new items get Backlog /
#      Blocked / Done according to rules 2-3 as they are added).
...
while IFS=$'\t' read -r number url state has_pending; do
  ...
  if [[ -z "$item_id" ]]; then
    # New to the board: add it and give it a starting status so it never sits in "No Status".
    if ! $DRY_RUN; then
