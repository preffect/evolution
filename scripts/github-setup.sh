...
#      push. Required status checks are NOT added here (see WORKFLOW.md §5).
#   4. scripts/project-sync.sh to put every issue on the board.
#
# Usage:  ./scripts/github-setup.sh [--repo owner/name] [--private] [--title "Display Title"]
#   --repo     default: <gh user>/<folder name>
#   --private  create a private repo (default public)
#   --title    project board + issue placeholder title (default from package.json name, Title Case)
...
[[ -n "$SLUG" ]] || SLUG="$NAME"
[[ -n "$TITLE" ]] || TITLE="$(echo "$NAME" | sed -E 's/[-_]+/ /g; s/\b(.)/\u\1/g')"
...
mkdir -p .github
python3 scripts/github/setup_project.py "$REPO" "$TITLE" "$SLUG" scripts/github/groundwork-issues.json .github/project.env

# ---- 3. ruleset on the default branch --------------------------------------
if ! gh api "repos/$REPO/rulesets" --jq '.[].name' 2>/dev/null | grep -qx "main: PR required, threads resolved"; then
  echo "==> Applying branch ruleset"
  gh api "repos/$REPO/rulesets" --method POST --input - >/dev/null <<'EOF'
{
  "name": "main: PR required, threads resolved",
  "target": "branch",
  "enforcement": "active",
...
if ! git cat-file -e origin/main:.github/project.env 2>/dev/null; then
  # main is PR-protected, so project.env lands via a PR. Generated ids, no reviewer role exists
  # yet at setup time — merged directly (WORKFLOW.md §6). Idempotent across re-runs.
  ...
  if [[ -z "$(gh pr list -R "$REPO" --head "$branch" --json number --jq '.[0].number')" ]]; then
    gh pr create -R "$REPO" -B main -H "$branch" \
      -t "chore: record GitHub project ids" -b "Generated by scripts/github-setup.sh. Used by scripts/project-sync.sh and scripts/issue-status.sh." >/dev/null
  fi
  gh pr merge "$branch" -R "$REPO" --squash --delete-branch >/dev/null
fi
git checkout -q main 2>/dev/null && git pull -q origin main
